...
RSpec.describe "bundle install with git sources" do
  ...
  describe "when specifying a revision" do
    ...
    it "works when the revision is a non-head ref" do
      ...
      git("update-ref -m \"Bundler Spec!\" refs/bundler/1 master~1", :dir => lib_path("foo-1.0"))
    ...
    it "does not download random non-head refs" do
      ...
      git("ls-remote .", :dir => Dir[home(".bundle/cache/git/foo-*")].first)
  ...
  it "handles repos that have been force-pushed" do
    ...
    git("reset --hard HEAD^", :dir => lib_path("forced-1.0"))
  ...
  context "with an extension" do
    ...
    it "does not use old extension after ref changes", :ruby_repo do
      ...
      2.times do |i|
        ...
        git("commit -m \"commit for iteration #{i}\" ext/foo.c", :dir => git_reader.path)
